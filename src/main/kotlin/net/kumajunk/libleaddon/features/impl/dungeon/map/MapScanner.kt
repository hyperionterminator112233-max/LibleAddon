...
import com.odtheking.odin.features.impl.dungeon.map.Unknown
...
object MapScanner {
    ...
    /**
     * 2つの部屋タイプ間の最短経路を計算する（BFS）
     * @param fromType 開始部屋のタイプ
     * @param toType 終了部屋のタイプ
     * @return 経路上の部屋名のリスト（開始・終了を含む）、見つからない場合は空リスト
     */
    fun getRouteBetween(fromType: RoomType, toType: RoomType): List<String> {
        val fromRoom = allRooms.values.find { it.data.type == fromType } ?: return emptyList()
        val toRoom = allRooms.values.find { it.data.type == toType } ?: return emptyList()

        // BFSで最短経路を探索
        val visited = mutableSetOf<MapRoom>()
        val queue = ArrayDeque<Pair<MapRoom, List<MapRoom>>>()
        queue.add(fromRoom to listOf(fromRoom))

        while (queue.isNotEmpty()) {
            val (current, path) = queue.removeFirst()
            if (current == toRoom) {
                return path.map { it.data.name }
            }
            ...
            // この部屋に隣接するドアを探す
            for (door in doors) {
                val adjacentRooms = door.rooms.map { it.owner }
