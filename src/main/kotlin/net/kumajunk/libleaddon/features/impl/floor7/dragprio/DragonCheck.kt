...
object DragonCheck {
    ...
    fun dragonUpdate(packet: ClientboundSetEntityDataPacket) {
        val entity = OdinMod.mc.level?.getEntity(packet.id) as? EnderDragon ?: return
    ...
    fun dragonSprayed(packet: ClientboundSetEquipmentPacket) {
        ...
        val sprayedEntity = OdinMod.mc.level?.getEntity(packet.entity) as? ArmorStand ?: return

        WitherDragonsEnum.entries.forEach { dragon ->
            val entity = OdinMod.mc.level?.getEntity(dragon.entityUUID ?: return@forEach) as? EnderDragon ?: return@forEach
            ...
            if (WitherDragons.sendSpray) {
                modMessage(
                    "§${dragon.colorCode}${dragon.name} §fdragon was sprayed in §c${
                        (WitherDragons.currentTick - dragon.spawnedTime).let {
